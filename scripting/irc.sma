...
public channel_commands(name[],command[])
{
	...
	if(equali(command,"@map"))
	{
		...
		format(temp,1024,"NOTICE %s :Current map: %s^r^n",name,mapname)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	if(equali(command,"@nextmap"))
	{
		new nextmap[51]
		get_cvar_string("amx_nextmap",nextmap,50)
		format(temp,1024,"NOTICE %s :next map: %s^r^n",name,nextmap)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	if(equali(command,"@timeleft"))
	{
		new timeleft[26]
		get_cvar_string("amx_timeleft",timeleft,25)
		format(temp,1024,"NOTICE %s :Time left: %s^r^n",name,timeleft)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	if(equali(command,"@ip"))
	{
		new ip[51]
		get_user_ip(0,ip,50)
		format(temp,1024,"NOTICE %s :Server IP: %s^r^n",name,ip)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	if(equali(command,"@players"))
	{
	...
	{
		startup_message(2,name)
	}
	if(command[0] == '@')
	{
		new adminaccess = is_irc_admin(name)
		if(adminaccess != -1)
			do_command(name,adminaccess,command,1)
	}
	return PLUGIN_CONTINUE
}

public private_commands(name[],command[])
{
	if(equali(command,"cmds") || equali(command,"info") || equali(command,"commands") || equali(command,"help") || equali(command,"!help") || equali(command,"!cmds") || equali(command,"!commands") || equali(command,"!info"))  //UPDATE AS ALONG
	{
		irc_cmd_list(name,1)
		return PLUGIN_CONTINUE
	}
	if(containi(command, "login") != -1)
	{
	...
	if(equali(command,"map"))
	{
		new mapname[51]
		get_mapname(mapname,50)
		format(temp,1024,"PRIVMSG %s :Current map: %s^r^n",chan,mapname)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	if(equali(command,"nextmap"))
	{
		new nextmap[51]
		get_cvar_string("amx_nextmap",nextmap,50)
		format(temp,1024,"PRIVMSG %s :Next map: %s^r^n",chan,nextmap)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	if(equali(command,"timeleft"))
	{
		new timeleft[26]
		get_cvar_string("amx_timeleft",timeleft,25)
		format(temp,1024,"PRIVMSG %s :Time left on map: %s^r^n",chan,timeleft)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	if(equali(command,"status"))
	{
		startup_message(1,name)
	}
	if(equali(command,"ip"))
	{
		new ip[51]
		get_user_ip(0,ip,50)
		format(temp,1024,"PRIVMSG %s :Server IP: %s^r^n",chan,ip)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	if(equali(command,"players"))
	{
		format(temp,1024,"PRIVMSG %s :Begin Players List^r^n",name)
		additem(temp)
		...
		for(new anum=0;anum<34;anum++)
		{
			...
			format(temp,1024,"PRIVMSG %s :#%d    %s    %s    %s^r^n",name,anum,authid,uname,ip)
			additem(temp)
		}
		format(temp,1024,"PRIVMSG %s :End Players List^r^n",name)
		additem(temp)
		return PLUGIN_CONTINUE
	}
	new adminaccess = is_irc_admin(name)
	if(adminaccess != -1)
		do_command(name,adminaccess,command,0)
	return PLUGIN_CONTINUE
}
